...
def main(timespan=4, model: Callable = AGDP):
    ...
    res = pd.DataFrame(
        np.array(res),
        columns=["init_loss", "trade reduction [%]", "alpha", "output"],
    )
    ...
    ax2 = ax.secondary_yaxis(
        location="right",
        functions=(
            lambda y: 100 * y / sum(nx.get_node_attributes(G, "GDP").values()),
            lambda y: y * sum(nx.get_node_attributes(G, "GDP").values()) / 100,
        ),
    )
    ax2.spines[["right", "top", "left", "bottom"]].set_visible(False)
    ax.set_xlabel("India, Pakistan initial loss [% GDP]")
    ax.set_ylabel("Global loss [$]")
    ax2.set_ylabel("Global loss [% GDP]")
    plt.savefig("./results/india_pakistan.png", bbox_inches="tight")
